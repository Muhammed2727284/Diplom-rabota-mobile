...
const VacancyCard = ({ item, onPress, isFavorite = false }) => {
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.7}>
      <View style={styles.header}>
        <View style={styles.titleWrap}>
          <Text style={styles.title} numberOfLines={2}>{item.title}</Text>
          <Text style={styles.company} numberOfLines={1}>{item.company_name || 'Компания'}</Text>
        </View>
        <FavoriteHeart targetType="VACANCY" targetId={item.id} initialActive={isFavorite} />
      </View>
...
const styles = StyleSheet.create({
  ...
  salary: { ...FONTS.bold, color: COLORS.primary, marginTop: SPACING.sm, fontSize: 15 },
  ...
  tagAccentText: { fontSize: 12, color: COLORS.primary, fontWeight: '500' },
});
