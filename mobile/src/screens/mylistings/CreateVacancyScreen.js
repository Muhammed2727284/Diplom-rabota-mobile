...
const CreateVacancyScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Успешно', 'Вакансия создана');
      navigation.goBack();
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder="Например: Frontend разработчик" placeholderTextColor={COLORS.textLight} />

      <Text style={styles.label}>Должность <Text style={styles.required}>*</Text></Text>
      <TextInput style={styles.input} value={position} onChangeText={setPosition} placeholder="Должность" placeholderTextColor={COLORS.textLight} />
      ...
      <TextInput style={[styles.input, styles.textArea]} value={description} onChangeText={setDescription} placeholder="Описание вакансии" placeholderTextColor={COLORS.textLight} multiline textAlignVertical="top" />

      <Text style={styles.label}>Требования</Text>
      <TextInput style={[styles.input, styles.textArea]} value={requirements} onChangeText={setRequirements} placeholder="Требования" placeholderTextColor={COLORS.textLight} multiline textAlignVertical="top" />

      <Text style={styles.label}>Условия</Text>
      <TextInput style={[styles.input, styles.textArea]} value={conditions} onChangeText={setConditions} placeholder="Условия работы" placeholderTextColor={COLORS.textLight} multiline textAlignVertical="top" />
      ...
      <TouchableOpacity style={[styles.submitBtn, loading && { opacity: 0.7 }]} onPress={handleSubmit} disabled={loading} activeOpacity={0.8}>
        {loading ? <ActivityIndicator color={COLORS.white} /> : <Text style={styles.submitText}>Создать вакансию</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
};
